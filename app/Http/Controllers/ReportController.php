...
class ReportController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // Get the selected store ID, month, and day from the request
        ...
        $dailySales = []; // Array to hold daily sales data
        ...
        if ($selectedMonth && $selectedDay !== 'all') {
            $dailySalesQuery = Sale::whereYear('date_of_transaction', Carbon::now()->year)
                ->whereMonth('date_of_transaction', $selectedMonth)
                ->whereDay('date_of_transaction', $selectedDay)
                ->where('sale_made', $selectedStoreId === 'all' ? $stores->pluck('id')->toArray() : $selectedStoreId);
            ...
            foreach ($dailySalesData as $sale) {
                ...
                foreach ($items as $item) {
                    ...
                    if (!isset($dailySales[$bundleId])) {
                        $dailySales[$bundleId] = 0;
                    }
                    $dailySales[$bundleId] += $subTotal; // Sum the subtotals for the day
                }
            }
        }

        return view('pages.reports', compact('stores', 'monthlySales', 'orderedItemsSales', 'quantityPerBundle', 'dailySales'));
    }
}
